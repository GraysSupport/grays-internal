...
export default function CollectionDetailPage() {
  ...
  const [lots, setLots] = useState([]); // G3: lots generated by this collection
  ...
  // G3: load lots generated by this collection (refreshes when completion state changes)
  useEffect(() => {
    if (!id) return;
    let alive = true;
    fetch(`/api/lots?collection_id=${id}`)
      .then((r) => (r.ok ? r.json() : []))
      .then((d) => { if (alive) setLots(Array.isArray(d) ? d : []); })
      .catch(() => { if (alive) setLots([]); });
    return () => { alive = false; };
  }, [id, collection?.lots_generated_at]);
    ...
    <>
      ...
      <div className="min-h-screen bg-gray-100 flex justify-center items-start p-6">
        <div className="bg-white p-6 rounded shadow-md w-full max-w-6xl space-y-6">
          ...
          {/* G3: Lot numbers generated by this collection (trace) */}
          <div className="rounded border bg-white">
            <div className="flex items-center justify-between p-3 border-b">
              <div className="font-semibold">
                Lot Numbers{' '}
                <span className="text-sm font-normal text-gray-500">
                  {collection?.lots_generated_at
              ...
              {isSuperadmin && collection?.status === 'Completed' && (
                <button
                  onClick={async () => {
                    const toastId = toast.loading('Syncing lots…');
                    try {
                      const res = await fetch(`/api/collections?resource=sync-lots&id=${id}`, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json', 'x-user-id': userIdHeader },
                      });
                      const data = await res.json().catch(() => ({}));
                      if (!res.ok) throw new Error(data?.error || 'Sync failed');
                      toast.success(`Lots synced (+${data.added || 0}, voided ${data.voided || 0}${data.blocked ? `, ${data.blocked} blocked` : ''})`, { id: toastId });
                      const r = await fetch(`/api/lots?collection_id=${id}`);
                      setLots(r.ok ? await r.json() : []);
                    } catch (err) {
                      toast.error(err.message || 'Sync failed', { id: toastId });
                    }
                  }}
                  className="rounded border px-3 py-1 text-sm hover:bg-gray-50"
                  title="Reconcile lots after editing items on a completed collection"
                >
                  Sync lots
                </button>
              )}
            ...
            {lots.length > 0 ? (
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {lots.map((l) => (
                      <tr key={l.lot_id}>
                        ...
                        <td className="px-3 py-2 font-mono">{l.product_sku}</td>
              ...
              <div className="p-3 text-sm text-gray-500">
                {collection?.lots_generated_at ? 'No lots.' : 'Not yet generated.'}
              </div>
            )}
          </div>
