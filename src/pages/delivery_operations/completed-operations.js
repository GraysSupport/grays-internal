...
export default function CompletedOperationsPage() {
  ...
  useEffect(() => {
    ...
    let mounted = true;
    const load = async () => {
      toast.loading('Loading completed operations...', { id: 'ops-load' });
      try {
        const res = await fetch('/api/workorder?status=Completed');
        const data = await res.json();
        if (!res.ok) throw new Error(data?.error || 'Failed to load');
        if (mounted) setRows(data || []);
        toast.success('Completed operations loaded', { id: 'ops-load' });
      } catch (e) {
        toast.error(e.message || 'Failed to load', { id: 'ops-load' });
      } finally {
          ...
          <div className="rounded-xl border bg-white">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200 table-fixed">
                ...
                <tbody className="divide-y divide-gray-100">
                  {loading && (
                    <tr>
                      <td colSpan={10} className="px-4 py-6 text-center text-sm">Loading…</td>
                    </tr>
                  )}
                  {!loading && pageRows.length === 0 && (
                    <tr>
                      <td colSpan={10} className="px-4 py-6 text-center text-sm">No completed work orders.</td>
                    </tr>
                  )}
                  {pageRows.map((w) => {
                    const itemsStr = formatItems(w.items);
                    ...
                    return (
                      <tr key={w.workorder_id} className="cursor-pointer align-top odd:bg-white even:bg-gray-50 hover:bg-gray-100" onClick={() => navigate(`/delivery_operations/workorder/${w.workorder_id}`)}>
                        ...
                        <td className="px-4 py-3 text-sm w-28">{w.delivery_suburb ?? '—'}</td>
                        <td className="px-4 py-3 text-sm w-20">{w.delivery_state ?? '—'}</td>
                        <td className="px-4 py-3 text-sm whitespace-normal break-words">{itemsStr}</td>
                        <td className="px-4 py-3 text-sm w-20">{w.salesperson ?? '—'}</td>
                        ...
                        <td className="px-4 py-3 text-sm w-28">{formatDate(w.estimated_completion)}</td>
                        <td className="px-4 py-3 text-sm whitespace-normal break-words w-40">{w.notes || '—'}</td>
                      </tr>
                    );
                  })}
                </tbody>
            ...
            <div className="flex items-center justify-between border-t p-3 text-sm">
              <div className="text-gray-600">
                Showing <span className="font-medium">{filtered.length === 0 ? 0 : (page - 1) * PAGE_SIZE + 1}</span> to <span className="font-medium">{Math.min(page * PAGE_SIZE, filtered.length)}</span> of <span className="font-medium">{filtered.length}</span>
              </div>
              <div className="flex items-center gap-1">
                <button disabled={page === 1} onClick={() => setPage((p) => Math.max(1, p - 1))} className="rounded-md border px-3 py-1 disabled:cursor-not-allowed disabled:opacity-50">← Previous</button>
                <span className="mx-2 rounded-md bg-gray-900 px-3 py-1 text-white">{page}</span>
                <button disabled={page >= totalPages} onClick={() => setPage((p) => Math.min(totalPages, p + 1))} className="rounded-md border px-3 py-1 disabled:cursor-not-allowed disabled:opacity-50">Next →</button>
              </div>
            </div>
          </div>
        </main>
