...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (method === 'POST') {
      ...
      const r = await client.query(
        ...
        INSERT INTO delivery (
          ...
          $1,$2,$3,$4,$5,$6,$7,$8, COALESCE($9, 'To Be Booked'::delivery_status_enum), $10,$11, NOW()
        )
        RETURNING delivery_id, workorder_id
        `,
        [
          invoice_id,
          Number(customer_id),
    ...
    if (method === 'PUT') {
        ...
        // Load current record to get workorder_id for logging
        const cur = await client.query(
            `SELECT delivery_id, workorder_id, delivery_status FROM delivery WHERE delivery_id = $1`,
            [id]
        );
        if (!cur.rows.length) {
